...
Resident engram allocation, CQ row gathering, key/value projection, convolution-ring lifecycle, and injection binding are isolated in `ResidentEngramState`; the main session supplies only shared matrix/projection access.
...
The TypeGPU-native migration now covers dynamic query/KV/attention parameter buffers, the primary resident f32/u32/i32 state allocations (lanes, QKV, attention, MLP, engram rings, logits, and KV caches), and the CQ, query normalization/RoPE, KV-store, attention-score, softmax/value-mixing, confidence, and constrained-selection bind-group contracts. `typegpu-parameters.ts` defines named `d.struct` schemas, creates root-owned typed storage buffers, and writes objects instead of manually indexed arrays. The resident core consumes them through a neutral parameter-factory interface and retains a raw factory for portability. TypeGPU `d.arrayOf` schemas allocate root-owned storage with the required raw copy flags, while a neutral resource factory preserves the reference path. Resource release now routes back through the creating factory: TypeGPU buffers are destroyed through their typed wrappers and raw buffers through `GPUBuffer.destroy()`, avoiding wrapper/raw lifecycle ambiguity. The CQ, query, KV-store, attention-score, softmax, confidence, and selection pipelines use explicit TypeGPU bind-group and pipeline layouts with typed mutability, runtime cache arrays, scales, and parameter schemas. Resident query normalization/RoPE and packed CQ matvec are TypeGPU `computeFn` pipelines with TypeGPU-owned workgroup memory, dependency resolution, and pipeline creation; their bodies remain inline WGSL so published builds do not require an application transform plugin. Command scheduling remains raw, and the adaptive operator backend retains its faster measured raw CQ pipeline. Schema tests lock buffer sizes and binding access, browser parity passes, and resident throughput remains about 54 tok/s.

`test:browser` bundles a real-browser suite and drives it through `Bun.WebView`. On macOS it uses WKWebView/WebGPU and verifies resident greedy text, confidence tolerance, constrained tool selection, KV-window wraparound with prefix sinks, repeated resets, float32-KV fallback, and cancellation. Without `NEEDLE_MODEL_PATH`, it downloads and caches the pinned model. The GitHub-hosted WKWebView currently has no WebGPU adapter, so CI uses `RESIDENT_ALLOW_NO_WEBGPU=1` to report a capability warning instead of a false test failure; full GPU assertions run on local or self-hosted compatible machines.
