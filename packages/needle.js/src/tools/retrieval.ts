...
import { type NeedleTool, serializeTools } from "./schema.js";
...
export function rankToolsBM25(query: string, tools: readonly NeedleTool[]): number[] {
  ...
  for (const word of queryWords) {
    const frequencies = documents.map((document) =>
      document.reduce((count, token) => count + Number(token === word), 0),
    );
    const documentFrequency = frequencies.reduce(
      (count, frequency) => count + Number(frequency > 0),
      0,
    );
    if (documentFrequency === 0) continue;
    const inverseDocumentFrequency = Math.log(
      1 + (tools.length - documentFrequency + 0.5) / (documentFrequency + 0.5),
    );
...
export function retrieveTools(
  ...
  if (tools.length <= maximum && (!tokenizer || tokenizer.encode(fullText).length <= budget))
    return [...tools];
  ...
  while (keep > minimum && tokenizer) {
    const selected = ranking
      ...
      .filter((tool): tool is NeedleTool => tool !== undefined);
    if (tokenizer.encode(serializeTools(selected)).length <= budget) return selected;
    keep--;
  }
  return ranking
    .slice(0, Math.max(minimum, keep))
    .map((index) => tools[index])
    .filter((tool): tool is NeedleTool => tool !== undefined);
}
