...
export function softmaxInPlace(values: Float32Array, length = values.length): Float32Array {
  ...
  for (let index = 1; index < length; index++)
    maximum = Math.max(maximum, values[index] ?? Number.NEGATIVE_INFINITY);
  let total = 0;
  ...
  for (let iteration = 0; iteration < iterations; iteration++) {
    for (let row = 0; row < size; row++) {
      ...
      for (let column = 1; column < size; column++)
        maximum = Math.max(maximum, values[offset + column] ?? Number.NEGATIVE_INFINITY);
      let sum = 0;
      for (let column = 0; column < size; column++)
        sum += Math.exp((values[offset + column] ?? 0) - maximum);
      const logSum = Math.log(sum) + maximum;
      for (let column = 0; column < size; column++)
        values[offset + column] = (values[offset + column] ?? 0) - logSum;
    }
    for (let column = 0; column < size; column++) {
      let maximum = values[column] ?? Number.NEGATIVE_INFINITY;
      for (let row = 1; row < size; row++)
        maximum = Math.max(maximum, values[row * size + column] ?? Number.NEGATIVE_INFINITY);
      let sum = 0;
      for (let row = 0; row < size; row++)
        sum += Math.exp((values[row * size + column] ?? 0) - maximum);
      const logSum = Math.log(sum) + maximum;
      for (let row = 0; row < size; row++)
        values[row * size + column] = (values[row * size + column] ?? 0) - logSum;
    }
  }
...
export function hadamardMlp(
  ...
  for (let index = 0; index < input.length; index++)
    values[index] = (input[index] ?? 0) * (d1[index] ?? 0);
